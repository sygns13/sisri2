...
data:{
   ...
   fillprogramassalud:{'id':'', 'nombre':'','descripcion':'','tipo':''},
...
methods: {
   ...
   create:function () {
       ...
       $(".form-control").css("border","1px solid #d2d6de");
       axios.post(url,{nombre:this.nombre, descripcion:this.descripcion, tipo:this.tipo}).then(response=>{
   ...
   borrar:function (programasalud) {
        ...
        swal.fire({
             title: '¿Estás seguro?',
             text: "¿Desea eliminar el Programa de Salud Seleccionado? -- Nota: este proceso no se podrá revertir.",
             type: 'info',
   ...
   editar:function (programasalud) {
       ...
       $("#boxTitulo").text('Programa de Salud: '+programasalud.nombre);
   ...
   update:function (id) {
       ...
       axios.put(url, this.fillprogramassalud).then(response=>{
           ...
           if(response.data.result=='1'){   
           this.getProgramasSaluds(this.thispage);
           this.fillprogramassalud={'id':'', 'nombre':'','descripcion':'','tipo':''};
           this.errors=[];
   ...
   nuevaProrroga:function (id) {
        ...
        $("#boxTituloProrroga").text('Submódulo: Gestión de Programas de Salud');
